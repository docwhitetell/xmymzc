    ...
    <div class="container">
        <div class="section-title text-center">
            <h2 class="animation {{$example->title_animation_delay ? "delay-".$example->title_animation_delay : ""}} {{$example->title_animation_duration ? $example->title_animation_duration : ""}}"
                ...
                style="font-size: {{$example->title_font_size .'px'}}; color: {{$example->title_font_color}};"
            >
                客户案例
            </h2>
            <p class="separator animation {{$example->desc_animation_delay ? "delay-".$example->desc_animation_delay : ""}} {{$example->desc_animation_duration ? $example->desc_animation_duration : ""}}"
               ...
               style="font-size: {{$example->desc_font_size .'px'}}; color: {{$example->desc_font_color}}; font-weight: 400"
        ...
        <div class="owl-carousel owl-theme">
            @foreach($example_data as $item)
                <div>
                    ...
                    <p style="font-size:{{$example->content_font_size.'px'}};color: {{$example->content_font_color}};font-weight: 500;margin-bottom: 0;">{{$item->title}}</p>
                    <p style="font-size:{{$example->content_font_size.'px'}};color: {{$example->content_font_color}};font-weight: 400; transform: scale(0.8)">{{$item->description}}</p>
                </div>
            @endforeach
        </div>
    </div>
